...
BASE add(BASE& m1,  string car, int num,string vio,string street) {
	BASE::iterator it;

	GAI::CRIMES.push_back(num, vio);

	it = m1.end();
	it->first.push_back(car);
	it->second.push_back(Crime(num, street));
...
int main() // ~Ассоциативные контейнеры
{
	...
	print(base);
	//save(base, "base.txt");
	base = add("k047ak", 8, "ул.Портовская");
